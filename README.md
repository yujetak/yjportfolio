# 🌟 탁유제(YJ TAK) | AI & SW 융합 엔지니어 포트폴리오

> **"SW로 길을 찾고, AI로 시야를 넓히다."**  
> 컴퓨터공학과 네트워크 연구 인프라 지식을 기반으로, 실무 비즈니스 환경에서 견고하게 작동하는 AI 서비스를 설계하고 구현하는 융합형 엔지니어 탁유제입니다.

---

## 🛠️ Tech Stacks (기술 스택)

### Frontend
- **Core**: React 19, TypeScript
- **Styling**: Vanilla CSS (Premium Glassmorphic Design)
- **Build Tool**: Vite

### Backend & AI / Data Engineering (주요 프로젝트 백그라운드)
- **Languages & Frameworks**: Python, Node.js (Express), Flask, EJS
- **Database / Cache**: PostgreSQL, MySQL, MariaDB, FAISS, Neo4j (Graph Database), Redis
- **AI & RAG**: LangChain, LangGraph, RAG, GraphRAG, PyTorch, TensorFlow, HuggingFace, XGBoost
- **DevOps & Cloud**: Docker, Kubernetes (GKE, minikube), AWS (EC2, RDS), GCP

---

## 🚀 Key Projects (대표 프로젝트 요약)

### 1. 대규모 언어 모델 기반 네트워크 설정 자동화 시스템 (연구과제)
- **Role**: NRF 석사과정생 연구장려금 지원사업 연구책임자 (PI)
- **Highlight**: RAG 기술을 접목하여 특화 언어인 P4_16 코드 템플릿 생성 연구, **2025 KCC 우수발표논문상 수상** 및 정보과학회 논문 게재 예정.

### 2. Nexus - 올인원 창업지원 서비스
- **Role**: 창업 시뮬레이션 기능 개발, 예측 모델 개발 및 인프라 배포 (PL, 5인)
- **Highlight**: 공공데이터(상업용 부동산 실거래가 API) 및 벡터 검색 기반 업종별 필수 설비 매핑, 카카오지도/소상공인 API 활용 실시간 상권 표시, XGBoost/CatBoost 기반 창업 성공 예측 AI 모델 구축, 백엔드 클라우드(Render, Huggingface 등) 배포 및 Docker/Kubernetes 도입.

### 3. DNN 기반 신용점수 예측 및 RAG 챗봇 서비스
- **Role**: DL / RAG Engineering (개인)
- **Highlight**: DNN 하이퍼파라미터 최적화 및 XGBoost와 성능 비교, Feature Engineering으로 데이터 편향 개선, 딥러닝 데이터셋(AIHub 금융 합성 데이터) 및 RAG 참조문서(KCB 개인신용평가체계 공시자료) 활용.

---

## 💻 Local Setup & Run (로컬 실행 방법)

본 프로젝트는 빠르고 가벼운 빌드 도구인 **Vite**를 기반으로 개발되었습니다.

### 1. 의존성 패키지 설치
```bash
npm install
```

### 2. 로컬 개발 서버 실행
```bash
npm run dev
```
개발 서버가 시작되면 브라우저에서 `http://localhost:5173`으로 접속할 수 있습니다.

### 3. 프로덕션 빌드 및 정적 파일 생성
```bash
npm run build
```
빌드가 성공적으로 완료되면 최적화된 파일들이 `dist/` 폴더 내에 생성됩니다.

---

## 📄 License & Copyright (저작권 및 라이선스)

Copyright © 2026 **YJ TAK (탁유제)**. All rights reserved.

- 본 포트폴리오 웹사이트의 디자인 및 소스 코드는 개인의 창작물입니다. 
- 비상업적인 학습 목적으로 소스 코드를 참고하시는 것은 가능하나, 텍스트 콘텐츠(프로젝트 설명, 연구 성과 등) 및 이미지 리소스를 무단 복제하여 본인의 포트폴리오로 도용하거나 상업적으로 재배포하는 행위는 엄격히 금지됩니다.
